...
export function ForecastView() {
  ...
  return (
    <div className="space-y-6">
      <div className="section-heading"><div><h2>See what the next year could hold.</h2><p className="muted">A twelve-month cash scenario using your saved income plan and recorded spending.</p></div></div>
      ...
      <div className="metric-grid">
        <article className="metric-card glass-panel p-5"><p className="muted text-sm">Starting liquid balance</p><p className="mt-2 text-2xl font-semibold tabular-nums">{money(projection.startingBalance)}</p><p className="muted mt-2 text-xs">Cash, checking and savings</p></article>
        <article className="metric-card glass-panel p-5"><p className="muted text-sm">Scenario income / month</p><p className="mt-2 text-2xl font-semibold tabular-nums">{money(projection.monthlyIncome)}</p><p className="muted mt-2 text-xs">{incomeMethod[0].toUpperCase() + incomeMethod.slice(1)} plan, including adjustments</p></article>
        <article className="metric-card glass-panel p-5"><p className="muted text-sm">Scenario spending / month</p><p className="mt-2 text-2xl font-semibold tabular-nums">{money(projection.monthlySpending)}</p><p className="muted mt-2 text-xs">Recorded average, including adjustments</p></article>
        <article className="metric-card glass-panel p-5"><p className="muted text-sm">Monthly cash change</p><p className="mt-2 text-2xl font-semibold tabular-nums" style={{ color: projection.monthlyNet < 0 ? "var(--danger, #b23423)" : "var(--accent, #0b6e3e)" }}>{projection.monthlyNet > 0 ? "+" : ""}{money(projection.monthlyNet)}</p><p className="muted mt-2 text-xs">Income less spending</p></article>
      </div>
      <section className="glass-panel p-5 sm:p-6" aria-labelledby="forecast-scenario-title">
        <div className="section-heading"><div><h3 id="forecast-scenario-title" className="text-lg font-semibold">Try a different month</h3><p className="muted mt-1 text-sm">What-if changes stay in this view. Your saved plan and transactions stay the same.</p></div></div>
      ...
      <section className="glass-panel overflow-x-auto" aria-label="Twelve-month forecast details">
